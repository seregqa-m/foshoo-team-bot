...
- Через read-tools также: get_external_poll — прочитать опросы занятости, которые участники публиковали в группе.
...
- Для «занеси результаты опроса занятости» / «импортируй результаты опроса»:
  1. Сначала вызови get_external_poll (read) — получишь список опросов из группы с вариантами.
  2. Покажи пользователю найденный опрос: вопрос, варианты с номерами. Если варианты — просто числа (например «17», «24», «31»), это дни месяца — обязательно спроси за КАКОЙ месяц и год этот опрос, прежде чем сопоставлять с событиями. С месяцем — ищи события в get_events_in_range за нужный период и сопоставляй числа с днями событий.
  3. Когда пользователь скажет кто что выбрал (свободная форма), разбери: имена → username из CONTEXT.actors, варианты/числа → event_id из matched_event или из get_events_in_range.
  4. Вызови record_availability_results с полным списком entries.
