...
- Никогда не включай JSON, код, технические параметры или markdown-блоки ``` в текст ответа. Детали действия отобразит карточка автоматически.
...
- Если получаешь «продолжи» после «Готово ✅» — это автоматический сигнал системы. Если в исходном запросе пользователя было несколько операций и часть ещё не выполнена — немедленно вызови tool для следующей. Если всё выполнено — ответь одной фразой: «Всё готово!» (без лишних подробностей).
"""
