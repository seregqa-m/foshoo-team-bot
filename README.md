...
4. Для проверки существующего спама откройте комментарий в группе обсуждений
   и **ответьте на него командой `/check_spam`** от своего обычного аккаунта,
   не от имени канала или анонимного администратора. Команда доступна только
   получателю уведомлений. В личку придёт карточка с результатом проверки
...
История старых сообщений автоматически не загружается: используйте `/check_spam`.
