...
финансовые записи могут `ADMIN_ID` и администраторы `GROUP_CHAT_ID`.
Результат проверки прав кешируется на 60 секунд. Если проверить состав нельзя,
доступ не предоставляется; заданный `ADMIN_ID` не зависит от Google Sheets.
